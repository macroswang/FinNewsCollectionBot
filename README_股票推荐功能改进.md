...
- **AI摘要解析失败**：AI生成的摘要中包含分隔符（如"---"），导致股票信息解析失败
- **股票推荐缺失**：当AI摘要无法正确解析时，系统无法提供股票推荐

### 修复方案

1. **改进解析逻辑**：在`extract_stock_recommendations_from_summary()`函数中添加分隔符过滤
2. **添加备用机制**：当AI摘要无法提取股票时，使用检测到的行业生成备用股票推荐
3. **增强调试信息**：添加详细的日志输出，便于问题排查
...
5. **备用机制**：当AI摘要无法解析时，系统会自动使用行业相关的备用股票推荐
6. **分隔符处理**：系统现在能够正确处理AI摘要中的各种分隔符和格式
